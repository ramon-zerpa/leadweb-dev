...
- `src/pages/index.astro` y `src/pages/es/index.astro` componen las portadas equivalentes EN/ES con layout, Nav, Hero, Stats, Work, Expertise, Founder, Process, Contact, CTA fijo WhatsApp y `BottomBar`.
...
- `src/components/Founder.astro`: sección semántica de confianza con H2, retrato y pie de foto, biografía, experiencia/formación y enlace externo a LinkedIn. `Picture` sirve `src/assets/portrait.png` en AVIF/WebP diferido. La imagen limpia usa relación `1 / 1.08`; `NeonBorder` a 8 s se separa 2 px y refuerza su halo. La grilla del retrato conserva 1,25 rem y el caption añade `margin-top: 1rem` con `padding: 0 2% 0 2%` en toda la escala. La sombra inferior del retrato es permanente; no hay transformaciones, escala ni intensificación al hover. Sin dependencias, solicitudes remotas ni JavaScript adicional.
...
- Páginas de privacidad y 404, imagen social, SEO final y verificación operativa de Formspree por página quedan fuera del alcance actual. La nota bajo el formulario no enlaza a privacidad para no crear rutas adicionales.
- `alternates` y sitemap i18n ya se generan para el par real `/` y `/es/`.
...
1. **Siguiente bloque solo bajo nueva petición.** La barra inferior ya está integrada; `Stats`, Work / DISOCA, Expertise, Founder, Process y Contact están completos en EN y ES; continuar una sección por vez.
