...
- **Estado del repositorio:** primer checkpoint del header/hero enviado a `origin` el 2026-09-06. El usuario autorizó un segundo checkpoint con todos los cambios pendientes, incluida la navegación interna, en la rama existente `master` y el `origin` indicado. El historial Git identifica la revisión exacta. Publicación en el hosting pendiente.
- **Estado global:** diseño v1 aprobado e implementación Astro por bloques pequeños. Firma de marca aprobada y typewriter valorado positivamente. Hero con iluminación circular amplia/desvanecida, brillos difusos en CTA superior/badge inspirados en el video de Pencil, degradado de marca en el typewriter y guion bajo titilante en la firma. Una sola escena final, sin etiqueta Structure/Experience, con mayor presencia visual y pausa compartida. `Stats`, Work/DISOCA y Expertise ya están integrados localmente; el siguiente bloque es Founder. Sitio completo y publicación pendientes.
...
Conservar el destino existente `https://formspree.io/f/xeoyljzz`. Mantener los campos de nombre, email, teléfono, servicio y mensaje. Antes de definir los atributos `name`, revisar los usados actualmente y sus automatizaciones para no romper compatibilidad; la etiqueta visible puede traducirse sin modificar la clave que espera la integración.
...
- El copy y el handoff proponen teléfono opcional. Comprobar el requisito operativo al integrar Formspree; este bloque de layout no cambia el formulario productivo.
...
| 3. Construcción | 120-180 min | Astro SSG, contenido tipado, idiomas, componentes, responsive y animación. | Diseño aprobado; continuar por bloques pequeños. | Base, cabecera, hero, `Stats`, Work/DISOCA y Expertise completados localmente. La franja replica los tres datos de escritorio y la síntesis móvil; Work conserva el navegador enmarcado y Expertise mantiene la lista de servicios del diseño. Continúan Founder y el resto de la portada. |
