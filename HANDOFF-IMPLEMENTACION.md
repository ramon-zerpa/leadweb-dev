...
## 41. PageSpeed Insights de producción y caché de assets — 2026-09-08

- La publicación manual en `https://leadweb.dev/` sirve la salida actual: `/`, `/es/`, `robots.txt`, schema y Reviews responden HTTP 200. Los recursos usan Brotli y LiteSpeed; HTML conserva el comportamiento de caché del hosting, CSS, fuentes e imágenes inicialmente tenían `max-age=604800`, mientras el script del hero no emitía `Cache-Control`.
- PSI oficial sin datos de campo aún —el dominio acaba de publicarse—: **móvil 99 / accesibilidad 100 / buenas prácticas 100 / SEO 100**, con FCP 1,3 s, LCP 1,4 s, TBT 40 ms, CLS 0 y Speed Index 3,2 s. Escritorio marca **100 / 100 / 100 / 100**, con FCP/LCP 0,3 s, TBT 0 ms, CLS 0,001 y Speed Index 0,6 s.
- Los avisos restantes de imágenes (47 KB) no puntúan y corresponden a candidatos AVIF/WebP de densidad retina, diferidos y fuera de la primera pantalla. Reducirlos a la resolución CSS deterioraría la nitidez aprobada sin mejorar LCP. Los avisos de animación tampoco penalizan el score; el TBT permanece bajo.
- `public/.htaccess`: añade `Cache-Control: public, max-age=31536000, immutable` exclusivamente para extensiones de assets generados por Astro. Sus hashes cambian con cada build, por lo que la caché larga es segura; no asigna caché larga a HTML, rutas, `robots.txt` o sitemap. Al publicar esta corrección se debe fusionar con cualquier `.htaccess` previo del document root, no reemplazar reglas de redirección existentes.
