...
- **Estado del repositorio:** primer checkpoint del header/hero enviado a `origin` el 2026-09-06; segundo checkpoint `97e5cdf` enviado a `origin/master` el 2026-09-07, con la navegación interna incluida. La sombra fija de Founder, corrección de la nota de privacidad, la portada funcional `/es/`, alternates/sitemap, SEO técnico y esta actualización documental están listos localmente para el siguiente checkpoint autorizado. Publicación en el hosting pendiente.
- **Estado global:** diseño v1 aprobado e implementación Astro por bloques pequeños. Firma de marca aprobada y typewriter valorado positivamente. Hero con iluminación circular amplia/desvanecida, brillos difusos en CTA superior/badge inspirados en el video de Pencil, degradado de marca en el typewriter y guion bajo titilante en la firma. Una sola escena final, sin etiqueta Structure/Experience, con mayor presencia visual y pausa compartida. `Stats`, Work/DISOCA, Expertise, Founder, Process, Reviews, Contact y una barra inferior están compuestos en las portadas `/` y `/es/`. Founder deja la imagen limpia, adopta una proporción más cuadrada, refuerza el borde neón de marca, proyecta una sombra permanente y no cambia al hover. Process conserva una pausa visual sobria con tres pasos, H2/lista ordenada y un contenedor neutro para la introducción; suma aire antes de su eyebrow en móvil/tablet y mantiene su título completo en una línea desde 1024 px. Reviews es una sección independiente con el criterio 05, H2 editorial a la izquierda y carrusel progresivo de testimonios completos 3/2/1 según el ancho; las tarjetas tienen alto contenido, atribución en dos columnas y controles a la derecha. Contact pasa al criterio 06 y conserva alternativas directas, sus redes más cercanas, más espacio antes de la ubicación, el formulario Formspree accesible y el servicio alineado con los demás campos, sin haber enviado una prueba real. `StickyWhatsApp.astro` añade una vía de conversión fija con borde orbital turquesa–cian–azul y una marca/anillo turquesa sobre ink: se oculta durante el hero, aparece al llegar a Stats y no obstruye el cierre. SEO técnico, semántica de la salida y recursos de build quedan auditados localmente; privacidad, 404, imagen social, prueba operativa de Formspree, medición PSI de producción y publicación siguen diferidos.
...
- **Ajuste visual vigente:** Founder usa la relación `1 / 1` del recurso maestro 1024 × 1024 en desktop, tablet y móvil. Sus candidatos optimizados pasan de 342/424 px a 342/424/636/848/1024 px para responder con nitidez a pantallas de alta densidad. Process conserva su `ol`, pero muestra iconos decorativos grandes y sin contenedor en lugar de números; se reduce el espacio entre Process y Reviews, se quita el divisor gris de Reviews y se aumenta a 2,5 rem el espacio entre su H2 y el carrusel.
...
- Español en Pencil: cabeceras de escritorio `qNP1M` y móvil `HZEXm`; no son maquetas completas. Los textos EN/ES de portada se componen en rutas funcionales equivalentes. Privacidad y 404 permanecen diferidas.
...
Ya existen Astro `^7.3.1`, TypeScript estricto, sitemap, idiomas/fuentes, recursos locales, copy EN/ES, layout global, CSS base, cabecera responsive y portadas completas. `src/pages/index.astro` y `src/pages/es/index.astro` componen las variantes equivalentes. La escena usa HTML/CSS y Web Animations API, sin nuevas dependencias. Ver resultados y límites en `HANDOFF-IMPLEMENTACION.md`.
...
| 3. Construcción | 120-180 min | Astro SSG, contenido tipado, idiomas, componentes, responsive y animación. | Diseño aprobado; continuar por bloques pequeños. | Base, cabecera, hero, `Stats`, Work/DISOCA, Expertise, Founder, Process, Reviews, Contact, CTA fijo WhatsApp y barra inferior completados localmente. La franja replica los tres datos de escritorio y la síntesis móvil; Work conserva el navegador enmarcado, Expertise mantiene la lista de servicios, Founder presenta el retrato y la biografía de la referencia con una imagen limpia, más cuadrada, borde neón, elevación/sombra, caption con margen/padding proporcional; Process conserva el recorrido de tres pasos de Pencil con H2/lista ordenada, `div` introductorio y aire responsive; Reviews es una sección propia con H2, seis testimonios EN/ES completos, carrusel 3/2/1, tarjetas compactas, atribución ordenada y controles a la derecha; Contact incorpora alternativas, LinkedIn/Instagram más cercanos, espacio adicional antes de ubicación, formulario responsive y grupo de servicio alineado; el CTA se revela desde Stats, mantiene el borde orbital de marca con icono/anillo turquesa y queda libre de solapamientos sobre el cierre; `BottomBar` aporta copyright y “With love from VE to USA.”. Continúan Footer ampliado y el resto de la portada. |
...
| 2026-09-08 | Portada ES, sombra fija y auditoría | La sombra inferior del retrato pasa a ser permanente y se retiran su elevación, escala e intensificación de halo al hover. La nota bajo Contact termina en la consulta, sin enlace de privacidad. Se compone `/es/` con los mismos componentes, copy localizado, anclas semánticas propias, switcher recíproco, canonical/hreflang y sitemap i18n. Build, estructura HTML, alternates, navegación sin hash, cambio ES→EN, consola y 1440/1024/768/390 px se verifican localmente. El CTA fijo usa `inert` mientras está oculto en hero, evitando focos en un control invisible. | Commit y push autorizados; no desplegar hosting ni enviar Formspree. |
| 2026-09-08 | Reviews bilingüe | Se incorporan seis testimonios aprobados, completos y visibles en el HTML. `Testimonials.astro` pasa a ser una sección hermana de Process: usa H2, el eyebrow 05, `aria-labelledby` y el enlace “Reviews” / “Reseñas”; Contact continúa con 06. `testimonials-carousel.ts` añade navegación manual, swipe, indicadores, estado accesible y 3/2/1 tarjetas según 1024/768/móvil. Las tarjetas reducen su alto mínimo a 21 rem, ordenan nombre/ubicación a la izquierda y dominio a la derecha, sin truncar las citas; los controles quedan a la derecha. Build y salida HTML EN/ES verifican estructura, navegación, jerarquía, numeración y ausencia de errores. | Revisión visual del usuario pendiente; sin páginas nuevas, commit, push ni despliegue autorizados. |
| 2026-09-08 | Densidad de Process, Reviews y Founder | Process sustituye los tres números visibles por iconos SVG decorativos y conserva el orden de la lista para accesibilidad. La transición hacia Reviews baja en Process y Reviews, se elimina el divisor gris y el carrusel recibe 2,5 rem después de su H2. Founder adopta `aspect-ratio: 1` en todos los breakpoints y `Picture` genera AVIF/WebP hasta 1024 px, con `sizes` reales de 424 px desde tablet para evitar ampliar el candidato de 424 px en pantallas retina. Build pasa con 27 archivos y cero diagnósticos. | Revisión visual del usuario pendiente; sin commit, push ni despliegue solicitados. |
| 2026-09-08 | SEO técnico y auditoría final local | Se añaden `robots.txt`, meta robots, Twitter y schema JSON-LD de `OnlineBusiness`, `Person`, `WebSite` y `WebPage`; permanecen canonicales, alternates y sitemap i18n. Se corrigen roles ARIA inválidos, orden de `figcaption`, nombres de landmarks, contraste y estilos inline de la salida. `npm run build`, `astro check`, HTML estricto EN/ES, smoke HTTP y auditoría estática pasan. El build no añade dependencias pesadas y conserva recursos comprimibles. | Medir PSI/CWV sobre el dominio HTTPS ya publicado: la estación local inyecta tráfico mediante una extensión ajena y hace inválida la puntuación Lighthouse. Imagen social, privacidad, 404, Formspree real, hosting y GSC quedan diferidos. |
